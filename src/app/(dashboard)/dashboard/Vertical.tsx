...
// Types Imports
import type { CardStatsVerticalProps } from './widgetTypes'
...
const CardStatVertical = (props: CardStatsVerticalProps) => {
  // Props
  const { title, stats, avatarIcon, avatarColor, subtitle, avatarSkin, avatarSize, moreOptions } = props
      ...
      <CardContent>
        <div className='flex justify-between items-center is-full mbe-5'>
          <CustomAvatar color={avatarColor} skin={avatarSkin} size={avatarSize} className='shadow-xs'>
            <i className={avatarIcon} style={{ fontSize: '2rem' }} />
          </CustomAvatar>
          <OptionMenu
            {...(moreOptions
              ? moreOptions
              : {
                  options: ['Refresh', 'Details', 'Hide Card'],
